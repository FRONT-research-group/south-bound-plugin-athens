...
#   filename:  1_API-Swaggers/EdgeCloud/code/API_definitions/Edge-Application-Management.yaml
#   timestamp: 2025-07-02T19:47:40+00:00
...
class AppId(RootModel[UUID]):
    ...
    def __eq__(self, other):
        return isinstance(other, AppId) and self.root == other.root


class AppInstanceId(RootModel[UUID]):
    '''
    A globally unique identifier associated with a running instance of an application.
    '''
    root: UUID = Field(
        ...,
        description=
    ...
    def __eq__(self, other):
        return isinstance(other, AppInstanceId) and self.root == other.root
...
class AppInstanceName(
        RootModel[constr(pattern=r'^[A-Za-z][A-Za-z0-9_]{1,63}$')]):
    '''
...
#     )
# Adapt to support both UUID and string for compatibility
class EdgeCloudZoneId(RootModel[Union[UUID, str]]):
    root: Union[UUID, str] = Field(
        ...,
        description=
        ("Unique identifier created by the Edge Cloud Platform to identify an\n"
         "Edge Cloud Zone within an Edge Cloud. Can be either UUID or string for compatibility."
         ),
    )
...
class RequiredResources(RootModel[Union[KubernetesResources, VmResources,
                                        ContainerResources,
                                        DockerComposeResources]]):
    root: Union[
        KubernetesResources, VmResources, ContainerResources,
        DockerComposeResources] = Field(
            ...,
            description=
...
class Uri(RootModel[str]):
    root: str = Field(
        ...
        examples=[
            'https://charts.bitnami.com/bitnami/helm/example-chart:0.1.0'
        ],
    )
...
class AccessEndpoint(RootModel[Union[AccessEndpoint1, AccessEndpoint2,
                                     AccessEndpoint3]]):
...
class ClusterInfo(BaseModel):
    name: str = Field(
        ..., description='Name of the Cluster, scoped to the Provider\n')
...
class AppsGetResponse(RootModel[List[AppManifest]]):
    root: List[AppManifest]


class AppsAppIdGetResponse(BaseModel):
    appManifest: Optional[AppManifest] = None


class AppinstancesGetResponse(RootModel[List[AppInstanceInfo]]):
    root: List[AppInstanceInfo]


class ClustersGetResponse(RootModel[List[ClusterInfo]]):
...
######  MINE ########
class AppInstanceAcceptedResponse(BaseModel):
    appInstanceId: str  # Or UUID, if preferred
